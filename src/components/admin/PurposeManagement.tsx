...
import {
  useFirestore, useCollection, useMemoFirebase, useUser,
...
export function PurposeManagement() {
  ...
  const [confirmDeletePurpose, setConfirmDeletePurpose] = useState<{ id: string; label: string } | null>(null);
  ...
  const handleAdd = async () => {
    ...
    await setDoc(doc(db, 'visit_purposes', id), { id, label, value: label, order, active: true });
    writeAuditLog(db, user, 'purpose.add', { detail: `Admin added purpose: "${label}"` });
    toast({ title: 'Purpose Added', description: `"${label}" is now visible in the kiosk.` });
    setNewLabel('');
  };
  ...
  const handleToggleActive = (p: VisitPurpose) => {
    ...
    toast({
      title: p.active ? 'Purpose Hidden' : 'Purpose Visible',
  ...
  const executeDeletePurpose = useCallback(() => {
    ...
    writeAuditLog(db, user, 'purpose.delete', { detail: `Admin removed purpose: "${confirmDeletePurpose.label}"` });
      ...
      <div className="space-y-4" style={{ fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <div style={card} className="p-4 sm:p-5">
          ...
          <div className="flex gap-2">
            <Input placeholder="New purpose label, e.g. Thesis Writing" value={newLabel}
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-slate-400 text-sm font-medium">
                {(purposes?.length ?? 0) === 0 ? 'No purposes yet. Add one above or seed the defaults.' : 'No matches.'}
              </p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow className="h-10 border-slate-100">
                  ...
                  <TableHead className="text-xs font-bold uppercase tracking-wide text-slate-500 bg-slate-50/80">Label</TableHead>
                  ...
                  <TableRow key={p.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors" style={{ height: 56 }}>
                      ...
                      {editingId === p.id ? (
                        <div className="flex items-center gap-2">
                          <Input value={editLabel} onChange={e => setEditLabel(e.target.value)}
                            ...
                            className="h-8 w-44 text-sm rounded-lg" autoFocus />
                          <button onClick={() => handleSaveEdit(p.id)} className="p-1.5 rounded-lg hover:bg-emerald-50 text-emerald-600"><Check size={14} /></button>
                          <button onClick={() => setEditingId(null)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400"><X size={14} /></button>
                        </div>
                      ) : (
                    ...
                    <TableCell>
                      <button onClick={() => handleToggleActive(p)}
                        ...
                        style={p.active ? { background: 'rgba(5,150,105,0.1)', color: '#059669' } : { background: 'rgba(100,116,139,0.1)', color: '#64748b' }}>
                    ...
                    <TableCell className="text-right pr-5">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => { setEditingId(p.id); setEditLabel(p.label); }}
